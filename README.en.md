...
Enterprise IdC accounts calling the Q endpoint require a `profileArn` — and it must be the real ARN (the Builder ID placeholder gets rejected due to identity mismatch) — yet the IdC token refresh response doesn't include it. Starting with this version the proxy **resolves it automatically**: before the account's first request, it calls `ListAvailableProfiles` to look up the real profileArn and writes it back to `credentials.json`, with no manual steps (us-east-1 / eu-central-1 endpoints are probed automatically with fallback; accounts with no Enterprise profile at all, such as pure Builder ID, are detected automatically and fall back to the official placeholder without repeated queries). If automatic resolution fails transiently (network hiccup etc.), the current request proceeds unaffected and the proxy retries on the next request.

To set one explicitly, the admin panel's "Add Account / Edit Account" dialog has a **Profile ARN** field; fill in a value like `arn:aws:codewhisperer:<region>:<account-id>:profile/<profile-id>` (obtainable from the Kiro IDE local cache or via `ListAvailableProfiles`; its region must match the account's `apiRegion`). An explicitly configured value always takes precedence. Social accounts don't need this field — the proxy fills in the shared Social ARN automatically.
...
**Q: Can I import an Enterprise IdC account automatically via AWS SSO login instead of pasting a refresh token?**

Yes. The "**AWS SSO Import**" button in the accounts toolbar uses the standard device authorization flow (RFC 8628, same as `aws sso login`): enter your portal Start URL and Auth Region, pick an API Region, and click Sign In — a browser authorization page opens and shows a User Code. After verifying the code matches, sign in and click Allow access; the proxy then fetches the token, validates it, resolves the real profileArn, and pulls the subscription tier automatically. **Email and nickname are identified for you**: sign-in registration prefers identity-claim scopes so the returned idToken fills in email and nickname (nickname falls back to email); explicitly typed values always win. Optional per-account proxy keeps imported records identical to manual adds; importing an already-present refresh token is deduplicated automatically.
